...
dataPath = [imagingPartitionPath '/ADNIdata/fMRI_ADNI2_ADNI3_initial_visit/'];
atlas = 'AAL3';
roiSignalsSuffix = 'NiftiPreprocessedAllBatchesNorm';
...
roiSignalsPath = [dataPath sprintf('/Results%s/ROISignals_%s_%s/', atlas, atlas, roiSignalsSuffix)];
...
outputPath = [dataPartitionPath '/UNSAM/Brain/Alzheimer/RMT_Full_Database/'];
classesToCompare = {'AD','CN','MCI'};
outputPath = fullfile(outputPath, atlas, roiSignalsSuffix, strjoin(classesToCompare,'_'), '/');
if ~isdir(outputPath)
    mkdir(outputPath)
end
%% READ CSV
subjectsInfo = readtable(csvFilename);
% Join all the MCI together:
subjectsInfo.ResearchGroup(contains(subjectsInfo.ResearchGroup, 'MCI')) = {'MCI'};
...
for i = 1 : numel(roiSignalsFilenames)
    ...
    if sum(strcmp(name, subjectsToExclude)) == 0
        ...
        elseif sum(strcmp(subjectsInfo.ResearchGroup(indexInTable), classesToCompare))>0
...
for i = 1 : size(coefficients,3)
    ...
    colorbar
    title(mdl.Coefficients.Properties.RowNames(i))
end
...
figure;
set(gcf, "Position", [50 50 3200 1200])
numCols = ceil(numBrainNetworks/2);
tiledlayout(2, numCols);
for i = 1 : numBrainNetworks
    nexttile
    violinplot(rmsBrainNetowrksSubjects(:,i),group);
    ylabel('RMS Brain Netowrk Signal');
    title(sprintf('Brain Netowrk %d', i))
end

saveas(gca, [outputPath 'ViolinPlotsRmsBrainNetwork'], 'png');
%% SHOW BRAIN NETWORKS
% Get AAL Rois first:
aalFilename = '/home/martin/data/UNSAM/Brain/Atlases/AAL3/AAL3v1_1mm.nii.gz';
aal = niftiread(aalFilename);
info=niftiinfo(aalFilename);
aal = double(aal);
% MNI 152 to have a structural reference:
mni152Filename = '/home/martin/data/UNSAM/Brain/Atlases/MNI152_T1_1mm.nii.gz';
mni152 = niftiread(mni152Filename);
mni152 = double(mni152);
infoMni152 = niftiinfo(mni152Filename);
% The is a small missmatch in dimensiones between the two atlases:
aal = imresize3(aal, size(mni152), 'nearest');
...
for i = 1 : numBrainNetworks
    ...
    for j = 1 : size(roisBrainNetworks{i},1)
        imageBrainNetowrk{i}(aal == roisBrainNetworks{i}(j)) = brainNetworks(roisBrainNetworks{i}(j),i);
    end
    ...
    % Center the range around zero:
    maxAbsValues = max(abs(maxValues), abs(minValues));
    % Scale the image for visualization with labeloverlay:
    scaledImage = uint8(round((imageBrainNetowrk{i}+maxAbsValues)./(2*maxAbsValues).*255));

    %labels = labels .* mask;
    m = 1;
    for l = 20 : 10 : size(imageBrainNetowrk{i},3)-10
    ...
    saveas(gcf, fullfile(outputPath, sprintf('VisBrainNetworks_%d',i)));

    close all

end
%% SHOW BRAIN NETWORKS
addpath('/home/martin/data/UNSAM/Tools/BrainNetViewer_20191031/')

%% SAVE ENVIRONMENT
save(fullfile(outputPath, 'matlabWorkspace'))
%% BUILD A CLASSIFIER
[trainedClassifier, validationAccuracy] = trainClassifierForRMT([rmsBrainNetowrksSubjects age], group)
%%
%% BUILD A CLASSIFIER
