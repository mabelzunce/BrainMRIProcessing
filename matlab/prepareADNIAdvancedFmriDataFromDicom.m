    ...
    for i = 3 : numel(dirPath)
        if(dirPath(i).isdir) 
            ...
            if sum(indicesRsFmri) > 0
                ...
                indicesMatchedName = find((strcmpi(casesToProcessAdvanced{end}, adniCollectionData.Subject)) > 0); 
...
%% PROCESS EACH CASE
for i = 1 : numel(casesToProcessAdvanced)    
    %% CONVERT DICOM TO NIFTI
    pathThisSubject = [dataPath casesToProcessAdvanced{i}];
    dicomPath = [pathThisSubject '/DICOM/'];
    niftiPathThisSubject = [niftiPath casesToProcessAdvanced{i} '/'];
    if ~exist(niftiPathThisSubject) || overwriteNifti
        dcmTagsTemp = dicm2nii(pathRsFmriAdvanced{i}, niftiPathThisSubject, format);
    end
    %% GET DATA FROM DICOM
    if exist([niftiPathThisSubject dcmHeadersFilename])
        ...
        %t1DparsfPathThisSubject = [t1DparsfPath '/' casesToProcessAdvanced{i} '/'];
        %% CHECK IF DATA ALREADY THERE
        fmriDparsfPathThisSubject = [fmriDparsfPath '/' casesToProcessAdvanced{i} '/'];
        if ~exist([fmriDparsfPathThisSubject niftiRsFmriFilenames{i} format]) || overwriteNifti
            %fieldmapMagDparsfPathThisSubject = [fieldmapMagDparsfPath '/' casesToProcessAdvanced{i} '/'];
            %fieldmapPhaseDparsfPathThisSubject = [fieldmapPhaseDparsfPath '/' casesToProcessAdvanced{i} '/'];
...
for s = 1 : numel(scanners)
    for g = 1 : numel(groups)
        ...
        for i = indicesSubgroup
            niftiPathThisSubject = [niftiPath casesToProcessAdvanced{i} '/'];
            fmriDparsfPathThisSubject = [dparsfThisGroupPath '/' casesToProcessAdvanced{i} '/'];
            mkdir(fmriDparsfPathThisSubject);
            copyfile([niftiPathThisSubject niftiRsFmriFilenames{i} format], fmriDparsfPathThisSubject);
        end
    end
